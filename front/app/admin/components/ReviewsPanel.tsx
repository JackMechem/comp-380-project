...
import { getAllReviews, deleteReview, ReviewPages } from "@/app/lib/AdminApiCalls";
...
const REVIEW_COLUMNS: Column<Review>[] = [
    ...
    { key: "account",       label: "Account",  defaultVisible: true,  render: (r) => getAccountLabel(r.account), minWidth: 160 },
    { key: "car",           label: "Car (VIN)",defaultVisible: true,  render: (r) => getCarVin(r.car), minWidth: 140 },
    { key: "stars",         label: "Stars",    defaultVisible: true,  render: (r) => <StarDisplay stars={r.stars} /> },
    { key: "title",         label: "Title",    defaultVisible: true,  render: (r) => r.title, minWidth: 160 },
    { key: "bodyOfText",    label: "Body",     defaultVisible: true,  render: (r) => r.bodyOfText, minWidth: 220 },
    { key: "rentalDuration",label: "Duration", defaultVisible: true,  render: (r) => `${r.rentalDuration} day${r.rentalDuration !== 1 ? "s" : ""}` },
...
const ReviewsPanel = () => {
    ...
    const fetchPage = useCallback(async (p: number, ps: number, isRefresh = false) => {
        ...
        try {
            const res: ReviewPages = await getAllReviews({ page: p, pageSize: ps });
            setReviews(res.data);
