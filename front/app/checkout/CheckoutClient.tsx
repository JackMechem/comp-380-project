...
const GuestEmailStep = ({
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (user) {
                // Check if a full account exists for this email
                const acctRes = await fetch(`/api/auth/check-account?email=${encodeURIComponent(email)}`, {
                    cache: "no-store",
                });
                const { exists } = acctRes.ok ? await acctRes.json() : { exists: false };
                ...
                onFound(email, mapUserToInfo({ ...user, email: user.email ?? email }), true);
            } else {
