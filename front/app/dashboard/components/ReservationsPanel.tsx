    ...
    return (
        <div className={styles.container}>
            ...
            <div className={styles.listGroup}>
                ...
                {[0, 1, 2].map((i) => (
                    <div key={i} className={styles.listRow} style={{ pointerEvents: "none" }}>
...
export default function ReservationsPanel() {
        ...
        fetch(`/api/accounts/${accountId}?parseFullObjects=true`)
            ...
            .then(async (account: Record<string, unknown>) => {
                ...
                const carResults = await Promise.all(
                    vins.map((vin) => fetch(`/api/cars/${vin}`).then((r) => (r.ok ? r.json() : null)).catch(() => null))
                );
    ...
    const fetchUserReviews = (acctId: number) => {
        fetch(`/api/reviews?account=${acctId}&objects-per-page=500`)
