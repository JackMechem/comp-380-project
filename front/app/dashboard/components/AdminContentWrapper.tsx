...
const LoadingScreen = ({ items }: { items: FetchItem[] }) => (
    <div style={{
        ...
        justifyContent: "center", minHeight: "40vh", gap: 32,
    }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 8, alignItems: "flex-start" }}>
            <p style={{ fontSize: "16pt", fontWeight: 700, color: "var(--color-foreground)", marginBottom: 8 }}>
            ...
            {items.map((item) => (
                <div key={item.key} style={{ display: "flex", alignItems: "center", gap: 12, fontSize: "11pt" }}>
                    <span style={{
                        ...
                        backgroundColor: item.status === "done" ? "rgba(34,197,94,0.12)"
                            : item.status === "error" ? "rgba(239,68,68,0.12)" : "transparent",
                        color: item.status === "done" ? "#22c55e"
                            : item.status === "error" ? "#ef4444" : "var(--color-foreground-light)",
                    }}>
                    ...
                    <span style={{
                        color: item.status === "done" ? "var(--color-foreground)"
                            : item.status === "error" ? "#ef4444" : "var(--color-foreground-light)",
                    }}>
                        {item.label}
                        {item.status === "done" && item.count !== undefined && (
                            <span style={{ color: "var(--color-foreground-light)", marginLeft: 6, fontSize: "9.5pt" }}>({item.count})</span>
                        )}
                        {item.status === "error" && <span style={{ marginLeft: 6, fontSize: "9.5pt" }}>— failed</span>}
                    </span>
                </div>
            ))}
        </div>
...
export default function AdminContentWrapper() {
    ...
    const refreshCars = async () => {
        setStatus("cars", "loading");
        try { const res = await getAllCars({ pageSize: 500 }); setCars(res.data); setStatus("cars", "done", res.data.length); }
        catch { setStatus("cars", "error"); }
    };
    const refreshReservations = async () => {
        setStatus("reservations", "loading");
        try { const res = await getAllReservations({ pageSize: 200 }); setRawReservations(res.data); setStatus("reservations", "done", res.data.length); }
        catch { setStatus("reservations", "error"); }
    };
    const refreshAccounts = async () => {
        setStatus("accounts", "loading");
        try { const res = await getAllAccounts({ pageSize: 200 }); setAccounts(res.data); setStatus("accounts", "done", res.data.length); }
        catch { setStatus("accounts", "error"); }
    };
    const refreshUsers = async () => {
        setStatus("users", "loading");
        try { const res = await getAllUsers({ pageSize: 500 }); setUsers(res.data); setStatus("users", "done", res.data.length); }
        catch { setStatus("users", "error"); }
    };
    ...
    switch (activeView) {
        ...
        case "view-reservations":
            return <AdminReservationsPanel reservations={reservations} accounts={accounts} onRefresh={refreshReservations} />;
        case "view-accounts":
        ...
        default:
            return <AdminDashboardPanel cars={cars} role={role ?? ""} />;
    }
}
